...
def register_denied_access(user_id):
    """Track rapid unauthorized /start attempts. Three attempts within 5 seconds trigger restriction."""
    now = _now()
    ...
    with get_connection() as c:
        ...
        if row and elapsed < 5:
            ...
            # A quiet period of 5 seconds starts a fresh sequence.
            attempts = 1
        ...
        c.execute(
            "INSERT INTO bot_access_attempts(user_id,attempts,notified,restricted,updated_at,last_attempt_at) VALUES(?,?,?,?,?,?) "
            "ON CONFLICT(user_id) DO UPDATE SET attempts=excluded.attempts, notified=excluded.notified, restricted=excluded.restricted, updated_at=excluded.updated_at, last_attempt_at=excluded.last_attempt_at",
            (user_id, attempts, notified, restricted, now, now),
        )
        c.commit()

    return {
        "attempts": attempts,
        "notify_owner": attempts == 3 and notified == 0,
