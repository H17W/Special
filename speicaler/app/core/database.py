...
def _initialize_database_at(path):
    ...
    with sqlite3.connect(path, timeout=30) as c:
        ...
        _ensure_column(c, "bot_access_attempts", "last_attempt_at", "TEXT")
...
def clear_all_private_data_once():
    ...
    with get_global_connection() as c:
        done = c.execute("SELECT value FROM settings WHERE key='private_data_reset_v21'").fetchone()
        if done and done["value"] == "1":
            return False
        c.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('private_data_reset_v21','1')")
        c.commit()
...
def _safe_delete_media_paths(paths):
    """Delete only media files belonging to Special's managed media directory."""
...
def delete_all_private_people():
    """Delete all stored private people/messages for the active profile only.

    This changes Special's local database only; it does not delete anything from Telegram.
    """
    with get_connection() as c:
        media = [r[0] for r in c.execute(
            "SELECT media_path FROM messages WHERE chat_id IN (SELECT user_id FROM private_chats WHERE is_bot=0)"
        ).fetchall()]
        private_ids = [r[0] for r in c.execute(
            "SELECT user_id FROM private_chats WHERE is_bot=0"
        ).fetchall()]
        if private_ids:
            marks = ','.join('?' for _ in private_ids)
            c.execute(f"DELETE FROM message_edit_history WHERE chat_id IN ({marks})", private_ids)
            c.execute(f"DELETE FROM messages WHERE chat_id IN ({marks})", private_ids)
            c.execute(f"DELETE FROM private_chats WHERE user_id IN ({marks})", private_ids)
            # Private AI logs are local stored data too; remove only the selected private users.
            c.execute(f"DELETE FROM ai_logs WHERE user_id IN ({marks}) AND (lower(COALESCE(chat_type,''))='private' OR source='user_account')", private_ids)
        c.commit()
    _safe_delete_media_paths(media)
    add_log("operation_log", "delete_all_private_people", f"count={len(private_ids)}")
    return len(private_ids)


def delete_private_person(user_id):
    """Delete one stored private person/messages for the active profile only.

    Returns False if the person is not stored. Telegram data is never modified.
    """
    uid = int(user_id)
    with get_connection() as c:
        row = c.execute(
            "SELECT user_id FROM private_chats WHERE user_id=? AND is_bot=0", (uid,)
        ).fetchone()
        if not row:
            return False
        media = [r[0] for r in c.execute(
            "SELECT media_path FROM messages WHERE chat_id=? OR user_id=?", (uid, uid)
        ).fetchall()]
        c.execute("DELETE FROM message_edit_history WHERE chat_id=? OR user_id=?", (uid, uid))
        c.execute("DELETE FROM messages WHERE chat_id=? OR user_id=?", (uid, uid))
        c.execute("DELETE FROM private_chats WHERE user_id=?", (uid,))
        c.execute(
            "DELETE FROM ai_logs WHERE user_id=? AND (lower(COALESCE(chat_type,''))='private' OR source='user_account')",
            (uid,),
        )
        c.commit()
    _safe_delete_media_paths(media)
    add_log("operation_log", "delete_private_person", f"user_id={uid}")
    return True
...
def unmute_user(user_id, username=None):
    """Remove a private mute robustly for the active Special profile.

    The numeric Telegram ID is authoritative. Username is only a compatibility
    fallback for older/stale mute records where the stored ID was not preserved
    correctly. This never searches or deletes another profile's data.
    """
    uid = int(user_id)
    with get_connection() as c:
        if username:
            normalized = str(username).lstrip("@").lower()
            cur = c.execute(
                "DELETE FROM muted_users WHERE user_id=? OR (lower(username)=? AND username IS NOT NULL)",
                (uid, normalized),
            )
        else:
            cur = c.execute("DELETE FROM muted_users WHERE user_id=?", (uid,))
        c.commit()
...
def upsert_private_chat(user_id, username, display_name, is_bot=False, is_deleted_user=False, last_seen=None):
    with get_connection() as c:
        c.execute(
            ...
            (user_id, username, display_name, last_seen or _now(), int(is_bot), int(is_deleted_user)),
        )
        c.commit()
...
def insert_messages_batch(records):
    """Insert many Telegram messages in one transaction for faster imports.

    Returns (inserted_count, inserted_records). Existing Telegram message identities
    are skipped without losing the original ordering/data.
    """
    if not records:
        return 0, []
    inserted_records = []
    with get_connection() as c:
        for record in records:
            (telegram_message_id, user_id, chat_id, direction, text, media_type,
             media_path, message_date) = record
            cur = c.execute(
        ...
        c.commit()
    return len(inserted_records), inserted_records
...
def increment_stats_bulk(user_id, records):
    """Update private-chat counters for a batch of newly inserted messages."""
    if not records:
        return
    mapping = {
        "photo": "photo_count", "video": "video_count", "sticker": "sticker_count",
        "gif": "gif_count", "audio": "audio_count", "file": "file_count",
    }
    counts = {
        "message_count": 0, "photo_count": 0, "video_count": 0, "sticker_count": 0,
        "gif_count": 0, "audio_count": 0, "file_count": 0, "link_count": 0,
    }
    import re
    for row in records:
        ...
        if mt in mapping:
            counts[mapping[mt]] += 1
        text = row[4]
        if text and re.search(r"https?://|t\.me/", text, re.I):
            counts["link_count"] += 1
    with get_connection() as c:
        c.execute(
            """UPDATE private_chats SET
               message_count=message_count+?, photo_count=photo_count+?, video_count=video_count+?,
               sticker_count=sticker_count+?, gif_count=gif_count+?, audio_count=audio_count+?,
               file_count=file_count+?, link_count=link_count+?
               WHERE user_id=?""",
            (counts["message_count"], counts["photo_count"], counts["video_count"],
             counts["sticker_count"], counts["gif_count"], counts["audio_count"],
             counts["file_count"], counts["link_count"], user_id),
        )
        c.commit()
...
def private_chats_count():
    with get_connection() as c:
        return int(c.execute("SELECT COUNT(*) AS n FROM private_chats WHERE is_bot=0 AND is_deleted_user=0").fetchone()["n"])
