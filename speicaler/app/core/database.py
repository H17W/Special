...
def clear_all_private_data_once():
    ...
    with get_global_connection() as c:
        done = c.execute("SELECT value FROM settings WHERE key='private_data_reset_v19'").fetchone()
        if done and done["value"] == "1":
            return False
        c.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('private_data_reset_v19','1')")
        c.commit()
