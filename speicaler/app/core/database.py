...
def register_denied_access(user_id):
    """Count denied /start attempts. Notify owner once, restrict on 3rd, ban on 4th."""
    now = _now()
    with get_connection() as c:
        ...
        if row:
            ...
            attempts, notified = 1, 0
        restricted = 1 if attempts >= 3 else 0
        c.execute(
            "INSERT INTO bot_access_attempts(user_id,attempts,notified,restricted,updated_at) VALUES(?,?,?,?,?) "
            "ON CONFLICT(user_id) DO UPDATE SET attempts=excluded.attempts, notified=excluded.notified, restricted=excluded.restricted, updated_at=excluded.updated_at",
            (user_id, attempts, notified, restricted, now),
        )
        c.commit()
    return {
        "attempts": attempts,
        "notify_owner": notified == 0,
