...
def _backup_media_rows():
    # Full backups contain only media that Special is meant to retain:
    # photos and Telegram voice notes. Telegram audio/music, videos, files,
    # stickers and GIFs are intentionally excluded.
    return _rows(
        "SELECT media_path FROM messages "
        "WHERE media_path IS NOT NULL AND media_type IN ('photo','voice')"
    )


def create_full_backup() -> list[Path]:
    """Create one backup archive and split it into exactly 3 equal byte parts.

    The three files are consecutive binary parts of the same ZIP archive.
    Reassembly is simply part1 + part2 + part3 in that order.
    """
    ...
    for index in range(3):
        ...
        part = BACKUP_DIR / f"special_backup_{stamp}.part{index + 1}.of3"
