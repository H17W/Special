...
def automation_menu():
    return kb([
        [
            ...
            btn("🔄 تحديث", "runtime_refresh"),
        ],
        [
            btn("🔇 كتم الخاص", "mute_private_user"),
            btn("🔊 فك كتم الخاص", "unmute_private_user"),
        ],
        [
            btn("🔇 كتم قروب/قناة", "mute_chat_user"),
            btn("🔊 فك كتم قروب/قناة", "unmute_chat_user"),
        ],
        [
...
def private_delete_all_confirm_menu():
    return kb([
        [
            btn("✅ نعم امسح الجميع", "private_delete_all_yes"),
            btn("❌ رفض", "private_delete_cancel"),
        ],
    ])
...
def storage_menu():
    return kb([
        [
            btn("📖 تصفح سجل المحادثات", "storage_chats:0"),
        ],
        [
        ...
        [
            btn("🔎 بحث عن محادثة للنسخ بالـID أو اليوزر", "backup_chat_search"),
        ],
        [
...
def broadcast_confirm_menu():
    return kb([
        [
            btn("✅ نعم أرسل", "broadcast_confirm_send"),
...
def mimic_menu(settings: dict):
    ...
    return kb([
        [
            btn(
                f"{mark('mimic_enabled')} تفعيل التقليد",
                "mimic_toggle"
            )
        ],
        [
            btn(
                f"{mark('mimic_name')} تقليد الاسم",
                "mimic_toggle:name"
            ),
            btn(
                f"{mark('mimic_photo')} تقليد الصورة",
                "mimic_toggle:photo"
            ),
        ],
        [
            btn(
                f"{mark('mimic_bio')} تقليد البايو",
                "mimic_toggle:bio"
            )
        ],
        [
...
def ai_menu(enabled: bool, trigger: str):
    ...
    return kb([
        ...
        [
            btn("✏️ إضافة اسم للذكاء", "ai_set_trigger"),
            btn("🗑️ حذف الاسم", "ai_clear_trigger"),
        ],
        [
