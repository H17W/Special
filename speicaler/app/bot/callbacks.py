...
async def feature_toggle(c):
    if not await guard(c, owner_only=True):
        return
    _, uid, key, page = c.data.split(':', 3)
    uid = int(uid); page = int(page)
    cur = is_feature_enabled(uid, key)
    set_feature_permission(uid, key, not cur)
    r = get_allowed_user(uid)
    if not r:
        await c.answer('المستخدم غير موجود', show_alert=True)
        return
...
async def feature_all(c):
    ...
    _, uid, flag = c.data.split(':')
    uid = int(uid)
    set_all_feature_permissions(uid, flag == '1')
    r = get_allowed_user(uid)
    if not r:
        await c.answer('المستخدم غير موجود', show_alert=True)
        return
    await c.answer('تم تحديث الصلاحيات')
    await show(
        c,
        f"🛠️ صلاحيات الميزات\n\n👤 {r['display_name'] or uid}",
        feature_permissions_menu(uid, get_feature_permissions(uid), 0)
    )
