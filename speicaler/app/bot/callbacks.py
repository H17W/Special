...
ACTION_KEYS = {
    ...
    "private_people":"private.people","storage_people":"storage.browse","storage_chat":"storage.browse","storage_browse":"storage.browse","sync_private_users":"private.people","import_messages":"private.import","private_search":"private.people","private_delete_menu":"private.people","private_delete_all_confirm":"private.people","private_delete_all_yes":"private.people","private_delete_cancel":"private.people","private_delete_one":"private.people","search_messages":"private.search","stats":"stats.people",
    "backup":"storage.backup","backup_choose":"storage.backup","backup_chat_search":"storage.backup","important_add":"important.add","important_list":"important.list",
...
@router.callback_query(F.data.startswith('storage_people:'))
async def storage_people(c):
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text='⬅️ السابق', callback_data=f'storage_people:{page-1}'))
    if (page + 1) * 10 < total:
        nav.append(InlineKeyboardButton(text='التالي ➡️', callback_data=f'storage_people:{page+1}'))
    if nav:
        buttons.append(nav)
...
async def storage_chat(c):
    ...
    if not r:
        ...
        return
    text = f"👤 {r['display_name'] or 'مستخدم'}\n🆔 ID: {uid}\n🔗 Username: {('@'+r['username']) if r['username'] else 'لا يوجد يوزر'}\n\n💬 الرسائل: {r['message_count']}\n🖼 الصور: {r['photo_count']}\n🎥 الفيديوهات: {r['video_count']}\n🃏 الملصقات: {r['sticker_count']}\n🎞 GIF: {r['gif_count']}\n🔗 الروابط: {r['link_count']}"
    await show(c, text, kb([[InlineKeyboardButton(text='📖 تصفح السجل', callback_data=f'storage_browse:{uid}:0')],[InlineKeyboardButton(text='💾 نسخ احتياطي', callback_data=f'backup:{uid}')],[InlineKeyboardButton(text='🔙 سجل المحادثات', callback_data='storage_people:0')]]))
...
def _message_datetime_sa(value):
    if not value:
        return None
    try:
...
def _media_label(media_type):
    return {
        ...
        'sticker': '🃏 ملصق',
        'gif': '🎞️ GIF',
    }.get(media_type, '📦 وسائط')


def _format_conversation(rows, person_name):
    lines = ['📖 سجل المحادثة', '']
    last_day = None
    for row in rows:
        dt = _message_datetime_sa(row['message_date'])
        if dt:
            day_key = dt.date()
            if day_key != last_day:
                if last_day is not None:
                    lines.append('')
                lines.append(f'{dt.day} {MONTHS_AR[dt.month]} {dt.year}')
                lines.append('')
                last_day = day_key
            time_text = dt.strftime('%H:%M')
        else:
            time_text = '--:--'
        sender = 'أنت' if row['direction'] == 'outgoing' else (person_name or 'هو')
        text = (row['text'] or '').strip()
        if not text:
            text = _media_label(row['media_type'])
        lines.append(f'{sender}  {time_text}')
        lines.append(text)
        lines.append('')
    return '\n'.join(lines).rstrip()
...
async def browse(c):
    ...
    uid_i, offset_i = int(uid), int(offset)
    rows = list_messages(uid_i, 10, offset_i)
    if not rows:
        await show(c, '📖 لا توجد رسائل محفوظة.', back(f'chat:{uid}'))
        return
    person = get_private_chat(uid_i)
    person_name = person['display_name'] if person else None
    text = _format_conversation(rows, person_name)
    nav = []
    if offset_i > 0:
        nav.append(InlineKeyboardButton(text='⬅️ الأقدم', callback_data=f'browse:{uid_i}:{max(0,offset_i-10)}'))
    if len(rows) == 10:
        nav.append(InlineKeyboardButton(text='الأحدث ➡️', callback_data=f'browse:{uid_i}:{offset_i+10}'))
    rows_kb = [nav] if nav else []
    rows_kb.append([InlineKeyboardButton(text='🔙 المحادثة', callback_data=f'chat:{uid_i}')])
    await show(c, text, InlineKeyboardMarkup(inline_keyboard=rows_kb))


@router.callback_query(F.data.startswith('storage_browse:'))
async def storage_browse(c):
    if not await guard(c, 'storage.browse'):
        return
    _, uid, offset = c.data.split(':')
    uid_i, offset_i = int(uid), int(offset)
    rows = list_messages(uid_i, 10, offset_i)
    if not rows:
        await show(c, '📖 لا توجد رسائل محفوظة.', back(f'storage_chat:{uid_i}'))
        return
    person = get_private_chat(uid_i)
    person_name = person['display_name'] if person else None
    text = _format_conversation(rows, person_name)
    nav = []
    if offset_i > 0:
        nav.append(InlineKeyboardButton(text='⬅️ الأقدم', callback_data=f'storage_browse:{uid_i}:{max(0,offset_i-10)}'))
    if len(rows) == 10:
        nav.append(InlineKeyboardButton(text='الأحدث ➡️', callback_data=f'storage_browse:{uid_i}:{offset_i+10}'))
    rows_kb = [nav] if nav else []
    rows_kb.append([InlineKeyboardButton(text='🔙 المحادثة', callback_data=f'storage_chat:{uid_i}')])
    rows_kb.append([InlineKeyboardButton(text='🗃 التخزين والنسخ الاحتياطي', callback_data='storage')])
    await show(c, text, InlineKeyboardMarkup(inline_keyboard=rows_kb))
