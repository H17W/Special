...
from app.core.database import init_database, get_allowed_user, get_feature_permissions, add_ai_log, get_setting, set_setting, add_bot_access_log, register_denied_access, mark_denied_access_notified, mark_denied_access_user_notified, reset_access_attempts, add_allowed_user, ensure_feature_permissions, remove_allowed_user
...
from .keyboards import main_menu, automation_menu, broadcast_menu, access_menu, broadcast_confirm_menu, ai_menu, kb, btn
...
async def start_handler(message: Message):
    ...
    if not allowed:
        # Unauthorized users must always receive the first denial message, even
        # when the bot is paused. Repeated /start attempts stay silent.
        ...
        if result["notify_user"]:
            try:
                ...
                # Do not mark the denial as delivered if Telegram rejected the send.
                raise
            else:
                mark_denied_access_user_notified(uid)
...
async def text_handler(message: Message):
    ...
    if uid in waiting_for_unmute:
        ...
        if not target:
            await message.answer("❌ أرسل Telegram ID أو @username صحيح.")
            return
        ok = unmute(target.id, getattr(target, "username", None))
    ...
    if action == "backup_chat_search":
        from app.core.database import search_private_chats
        from app.core.exporter import create_chat_backup
        raw = (message.text or "").strip()
        pending.pop(uid, None)
        target = None
        try:
            ...
            rows = search_private_chats(raw, 5)
            if rows:
                target = int(rows[0]["user_id"])
        if not target:
            await message.answer("❌ ما لقيت محادثة محفوظة بهذا الـID أو اليوزر.", reply_markup=kb([[btn("🔙 التخزين والنسخ الاحتياطي", "storage")]]))
            return
        path = create_chat_backup(int(target))
        if not path:
            await message.answer("❌ هذه المحادثة غير موجودة في التخزين.", reply_markup=kb([[btn("🔙 التخزين والنسخ الاحتياطي", "storage")]]))
            return
        try:
            await message.answer_document(FSInputFile(path), caption=f"💾 تم إنشاء نسخة المحادثة {int(target)}")
        finally:
    ...
    if action == "private_delete_one":
        ...
        try:
            ...
            try:
                ...
                target = None
        if not target:
            await message.answer("❌ أرسل Telegram ID رقمي أو @username صحيح.", reply_markup=private_menu())
            return
        from app.core.database import delete_private_person
        deleted = delete_private_person(int(target))
        if not deleted:
            await message.answer("ℹ️ هذا الشخص غير موجود في قائمة الخاص المحفوظة داخل Special.", reply_markup=private_menu())
            return
        await message.answer(f"✅ تم مسح الشخص ID: {int(target)} من Special فقط\n\nلم يتم حذف أو حظر أي شيء من Telegram.", reply_markup=private_menu())
        return
    ...
    if action == "access_add":
        ...
        try:
            ...
            display_name = " ".join(x for x in (first_name, last_name) if x).strip() or None
        except Exception:
            pass
        add_allowed_user(target, username, display_name or str(target), None)
