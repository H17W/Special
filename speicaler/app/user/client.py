...
from app.user.moderation import check_muted, mute, unmute, mute_in_chat, unmute_in_chat, is_muted_in_chat
from app.user.features.moderation_commands import handle_moderation_command
...
async def send_text_chunks(text, recipient_id=None):
    recipient_id = int(recipient_id or active_bot_user_id() or 0)
    if not recipient_id:
        return
    for i in range(0, len(text or ""), 4000):
        await bot.send_message(recipient_id, text[i:i + 4000])


def _is_temporary_media(message):
    """Return True only for Telegram media marked as self-destructing/temporary.

    Normal photos/videos/files must never be pulled merely because they were
    replied to. Telegram exposes the TTL on different layers depending on the
    media/update type, so check the known Telethon fields defensively.
    """
    if not message or not getattr(message, "media", None):
        return False
    candidates = (
        getattr(message, "ttl_period", None),
        getattr(message, "ttl_seconds", None),
        getattr(getattr(message, "media", None), "ttl_seconds", None),
        getattr(getattr(message, "media", None), "ttl_period", None),
    )
    return any(value is not None and int(value) > 0 for value in candidates if str(value).lstrip("-").isdigit())


async def save_replied_media(event, replied, sender, recipient_id=None):
    recipient_id = int(recipient_id or active_bot_user_id() or 0)
    if not replied or not replied.media or not recipient_id or not _is_temporary_media(replied):
        return False
    ...
    try:
        ...
        f = FSInputFile(path)
        if replied.photo:
            await bot.send_photo(recipient_id, f, caption=caption, reply_markup=markup)
        elif replied.video:
            await bot.send_video(recipient_id, f, caption=caption, reply_markup=markup)
        elif replied.voice:
            await bot.send_voice(recipient_id, f, caption=caption, reply_markup=markup)
        elif replied.audio:
            await bot.send_audio(recipient_id, f, caption=caption, reply_markup=markup)
        else:
            await bot.send_document(recipient_id, f, caption=caption, reply_markup=markup)
        return True
...
async def _send_edit_notification(sender, old_text, new_text, message_id, recipient_id=None):
    recipient_id = int(recipient_id or active_bot_user_id() or 0)
    if not recipient_id or get_setting("notify_edited", "1") != "1":
        return
    ...
    try:
        await bot.send_message(recipient_id, body[:3900], reply_markup=markup)
    except Exception:
        pass


async def _send_delete_notification(row, recipient_id=None):
    recipient_id = int(recipient_id or active_bot_user_id() or 0)
    if not recipient_id or get_setting("notify_deleted", "1") != "1":
        return
...
async def incoming_edit(event):
    ...
    if mark_message_edited(event.id, event.chat_id, "incoming", event.raw_text):
        await _send_edit_notification(sender, old_text, event.raw_text, event.id, active_bot_user_id())
            ...
            for row in stored:
                ...
                if mid not in current_ids and row["deleted_at"] is None:
                    if message_marked_deleted(mid, user_id, "incoming"):
                        await _send_delete_notification(row, active_bot_user_id())
        except Exception as exc:
...
async def outgoing(event):
    ...
    if event.is_private and _runtime_feature_enabled("private.own_messages"):
        ...
        if event.is_reply and _runtime_feature_enabled("automation.save_media"):
            ...
            if replied and replied.media and _is_temporary_media(replied):
                await save_replied_media(event, replied, target, active_bot_user_id())
...
async def runtime_watchdog():
    ...
    while True:
        ...
        try:
            clients = [(int(ADMIN_ID), client)] if ADMIN_ID else []
            clients.extend((uid, uc) for uid, uc in USER_CLIENTS.items())
            for bot_uid, uc in clients:
                set_active_client(uc, bot_uid)
                if not uc.is_connected():
                    async with WATCHDOG_LOCK:
                        await uc.connect()
                        add_log("operation_log", "telethon_reconnect", f"تمت إعادة الاتصال تلقائيًا للحساب {bot_uid}")
                if audit_tick >= 2 and _runtime_feature_enabled("automation.deleted_log"):
