...
async def outgoing(event):
    ...
    if lower.startswith((".فك", "فك", "/فك", ".unmute", "unmute", "الغاء الكتم", "إلغاء الكتم", "/الغاء الكتم", "/إلغاء الكتم")):
        ...
        # In groups/channels, allow unmute without replying to the deleted message:
        # الغاء الكتم 123456789 or الغاء الكتم @username
        if argument:
            try:
            ...
            if not target4 and not event.is_private:
                ...
                if row4:
                    try:
                        target4 = await active_client().get_entity(int(row4["user_id"]))
                    except Exception:
                        target4 = None
            if not target4:
            ...
            if not event.is_reply:
                await event.edit("⚠️ استخدم الغاء الكتم + ID أو @username أو بالرد على رسالة المستخدم")
                return
            replied = await event.get_reply_message()
            target4 = await replied.get_sender() if replied else None
            # In channels and some group updates Telegram may omit the sender entity
            # even though sender_id is present. The mute record only needs the numeric ID.
            if not target4 and replied and getattr(replied, "sender_id", None):
                class _Target:
                    id = int(replied.sender_id)
                target4 = _Target()
            if not target4:
                await event.edit("⚠️ لم أستطع تحديد صاحب الرسالة")
                return
        ...
        if event.is_private:
            ok = unmute_user(target4.id)
        else:
            ok = unmute_in_chat(event.chat_id, target4.id)
