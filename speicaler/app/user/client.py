...
async def is_special_or_bot_user(entity):
    ...
    if getattr(entity, "deleted", False):
        return True
    try:
        special_bot = await bot.get_me()
        if getattr(entity, "id", None) == getattr(special_bot, "id", None):
            return True
    except Exception:
        pass
    return False
...
async def sync_all_private_users():
    ...
    async for dialog in active_client().iter_dialogs():
        if not dialog.is_user:
            continue
        user = dialog.entity
        if user.id == me.id or await is_special_or_bot_user(user):
            skipped += 1
            continue
...
async def import_all_private_history():
    async with IMPORT_LOCK:
        ...
        async for dialog in active_client().iter_dialogs():
            ...
            if entity.id == me.id or await is_special_or_bot_user(entity):
                continue
            chats += 1
    ...
    if lower.startswith((".فك", "فك", "/فك", ".unmute", "unmute", "الغاء الكتم", "إلغاء الكتم", "/الغاء الكتم", "/إلغاء الكتم")):
        ...
        if argument:
            ...
            if target4_id is None:
                ...
                return
            if target4 is None:
                target4 = type("ReplyTarget", (), {"id": target4_id})()

        if event.is_private:
            ok = unmute_user(int(target4.id))
        else:
            ok = unmute_in_chat(int(event.chat_id), int(target4.id))
