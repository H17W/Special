...
async def start(m:Message):
    uid=identity(m); db.upsert_user(uid,m.from_user.username,m.from_user.first_name)
    ...
    if payload and not db.access_active(uid):
        ...
        if linked is not None:
            _, grant_seconds = linked
            db.run("UPDATE users SET status='active',expires_at=?,updated_at=? WHERE id=?",(int(time.time())+grant_seconds,int(time.time()),uid))
    if not is_owner(uid) and not db.access_active(uid):
        duration=abusive_start(uid)
        if duration==-1:
            db.restrict(uid,permanent=True,reason='Repeated /start abuse')
        elif duration>0:
            db.restrict(uid,duration,reason='Repeated /start')
        db.log(uid,'start','unauthorized')
        await m.answer('⛔ غير مصرح لك باستخدام Special.',reply_markup=access_request()); return
    if not db.unrestricted(uid):
        rem=db.remaining_restriction(uid); await m.answer('⛔ حسابك مقيّد حالياً.' + (f' المدة المتبقية {rem} ثانية.' if rem>0 else '')); return
    await m.answer('🧠 أهلاً بك في Special\n\nاختر من لوحة التحكم:',reply_markup=main_menu(is_owner(uid)))
