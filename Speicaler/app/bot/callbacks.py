...
from app.user.client import client as user_client, import_all_private_history, refresh_special, sync_all_private_users
...
ACTION_KEYS={
 "mute_user":"automation.mute","unmute_user":"automation.unmute","runtime_refresh":"automation.monitor","muted_list":"automation.muted_list","rules":"automation.rules","operation_log":"log.operations",
 "private_people":"private.people","sync_private_users":"private.people","import_messages":"private.import","search_messages":"private.search","stats":"stats.people","backup":"storage.backup","backup_choose":"storage.backup","important_add":"important.add","important_list":"important.list",
...
async def main(c):
    # Returning to the main menu must always work. Owners bypass the
    # permission database; regular users get their enabled feature set.
    uid = user_id(c)
    if not await guard(c):
        return
    if is_admin(uid):
        markup = main_menu(None)
    else:
        markup = main_menu(uid, get_feature_permissions(uid))
    await show(c,'🧠 أهلاً بك في Special\n\nلوحة التحكم الرئيسية:',markup)

@router.callback_query(F.data=='private_commands')
async def private_commands(c):
    if not await guard(c,'automation'): return
    text=(
        "📖 أوامر الخاص\n\n"
        "🔇 كتم — رد على رسالة الشخص واكتب كتم\n"
        "🔊 الغاء الكتم — رد على رسالته واكتب الغاء الكتم\n"
        "⭐ مهم — رد على أي رسالة واكتب مهم لحفظها\n"
        "🗑️ /بداية — رد على أول رسالة في النطاق\n"
        "🗑️ /نهاية — رد على آخر رسالة وسيتم حذف النطاق\n"
        "👤 تقليد — رد على رسالة المستخدم أو اكتب منشنه\n"
        "↩️ الغاء التقليد — يرجع بيانات حسابك المحفوظة قبل التقليد\n"
        "🔄 تحديث — تحديث الاتصال وإعادة جلب صورة وبيانات الشخص المقلد\n\n"
        "ملاحظة: المسح والكتم والتقليد تعمل في الخاص والقروبات والقنوات حسب صلاحيات حسابك.\n"
        "الرد على الرسالة مطلوب في كتم ومسح الرسائل."
    )
...
async def runtime_refresh(c):
    if not await guard(c,'automation'): return
    try:
        refreshed = await refresh_special()
        await show(c, '✅ تم تحديث البوت بنجاح\n\n' + ('🖼️ تم تحديث بيانات وصورة التقليد الحالي.' if refreshed.get('mimic_refreshed') else '🟢 تم فحص الاتصال وقاعدة البيانات والقائمة الخاصة بنجاح.'), automation_menu())
...
async def sync_private_users_cb(c):
    ...
    try:
        ...
        await c.message.answer(f"✅ تم تحديث قائمة الخاص\n\n➕ المضافون: {result['added']}\n🔄 الموجودون وتم تحديثهم: {result['updated']}\n⏭️ المتجاهلون: {result['skipped']}")
    except Exception as exc:
        await c.message.answer(f'❌ تعذر جلب المستخدمين: {type(exc).__name__}')
    await c.message.answer('📥 إدارة الخاص', reply_markup=private_menu())
    await c.answer()

@router.callback_query(F.data=='import_messages')
async def import_messages(c):
    if not await guard(c,'private.import'): return
    await c.message.edit_text('📥 جاري استيراد الرسائل القديمة من محادثات الخاص...')
    try:
        result = await import_all_private_history()
        await c.message.answer(
            '✅ تم استيراد الرسائل بنجاح\n\n'
            f"👥 المحادثات: {result['chats']}\n"
            f"💬 الرسائل الجديدة التي تمت إضافتها: {result['messages']}\n"
            f"⏭️ الرسائل الموجودة مسبقًا: {result['skipped']}"
        )
    except Exception as exc:
        await c.message.answer(f'❌ تعذر استيراد الرسائل: {type(exc).__name__}')
    await c.message.answer('📥 إدارة الخاص', reply_markup=private_menu())
    await c.answer()
