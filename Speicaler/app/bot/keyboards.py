...
def automation_menu():
    return kb([
        [btn("📖 أوامر الخاص", "private_commands"), btn("🔄 تحديث", "runtime_refresh")],
        [btn("🔇 كتم مستخدم", "mute_user"), btn("🔊 إلغاء كتم", "unmute_user")],
        [btn("👥 المكتومون", "muted_list")],
...
def storage_menu():
    return kb([
        [btn("📖 تصفح سجل المحادثات", "private_people")],
...
def feature_permissions_menu(user_id, permissions, page=0, per_page=10):
    ...
    nav = []
    if page > 0:
        nav.append(btn("⬅️ السابق", f"fpp:{user_id}:{page - 1}"))
    if end < total:
        nav.append(btn("التالي ➡️", f"fpp:{user_id}:{page + 1}"))
    if nav:
        rows.append(nav)
