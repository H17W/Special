...
async def _backup_current_profile():
    ...
    backup = {"active": True, "first_name": first, "last_name": last, "about": getattr(me, "about", None) or "", "photo": None, "created_at": datetime.now(timezone.utc).isoformat()}
...
async def _apply_mimic_target(target):
    ...
    about = await _fresh_about(refreshed)
...
async def deleted_event(event):
    ...
    rows = get_messages_by_telegram_ids(ids, chat_id=chat_id) if chat_id is not None else []
    if not rows:
        for mid in ids:
            rows.extend(get_private_records_by_message_id(mid))
    seen = set()
    for row in rows:
        if row["id"] in seen:
            continue
