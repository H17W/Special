...
def is_temporary_media(message):
    if not message or not message.media: return False
    # Telegram self-destruct media exposes a TTL on the message/media object.
    values=[getattr(message,'ttl_period',None), getattr(message.media,'ttl_seconds',None), getattr(message.media,'ttl_period',None)]
    return any(v is not None and int(v or 0)>0 for v in values)

PROFANITY_PATTERNS={
    "كلمات_مسيئة_عامة": ["ياحمار","يا غبي","تافه","قذر","كلب","حمار","غبي","حقير","سافل","وسخ","تافه","منحط"],
}

def contains_profanity(text):
    normalized=re.sub(r"[\s\u200b]+","",(text or "").casefold())
    return any(w.replace(' ','').casefold() in normalized for words in PROFANITY_PATTERNS.values() for w in words)
...
async def archive_media(event, user_id, direction):
    ...
    """
    if not event.message or not event.message.media or direction != "in" or not is_temporary_media(event.message):
        return None
...
async def _resolve_target(event):
    ...
    match = re.search(r"@([A-Za-z0-9_]{3,32})", raw)
    if match:
        try: return await client.get_entity(match.group(1))
        except Exception: return None
    if raw.split() and raw.split()[-1].lstrip('-').isdigit():
        try: return await client.get_entity(int(raw.split()[-1]))
        except Exception: return None
    return None
...
async def incoming(event):
    ...
    if event.is_private:
        ...
        if get_setting("private_lock", "0")=="1" and not is_private_lock_exception(sender.id):
            try: await event.delete()
            except Exception: pass
            return
        if get_setting("profanity_filter", "0")=="1" and contains_profanity(event.raw_text):
            try: await event.delete()
            except Exception: pass
            add_log("security_log","profanity_deleted",f"user={sender.id}")
            return
...
async def outgoing(event):
    ...
    if event.is_private:
        ...
        inserted_id, inserted = insert_message_once(event.id, target.id, event.chat_id, "outgoing", event.raw_text, media_type(event.message), None, str(event.date))
        if inserted:
            increment_stats(target.id, media_type(event.message), has_link(event.raw_text))

    if lower in {"الكيبورد", "/الكيبورد", "keyboard", "/keyboard"}:
        if bot is not None:
            from app.bot.keyboards import main_menu
            # Only the account owner and developer are allowed to use the inline controls.
            try:
                sent=await bot.send_message(int(event.chat_id), "⌨️ واجهة Special — التفاعل مسموح لصاحب الحساب فقط.", reply_markup=main_menu(None))
                add_keyboard_session(event.chat_id,sent.message_id,int(get_account_id()))
            except Exception: pass
        return
