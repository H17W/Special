...
class DB:
    ...
    def _init(self):
        with self.conn() as c:
            ...
            CREATE TABLE IF NOT EXISTS message_archive(chat_id INTEGER NOT NULL,message_id INTEGER NOT NULL,sender_id INTEGER,sender_name TEXT,sender_username TEXT,kind TEXT,text TEXT,media_type TEXT,media_path TEXT,created_at INTEGER NOT NULL,edited_at INTEGER,deleted_at INTEGER,PRIMARY KEY(chat_id,message_id));
            CREATE INDEX IF NOT EXISTS idx_archive_chat_time ON message_archive(chat_id,created_at,message_id);
            CREATE INDEX IF NOT EXISTS idx_archive_sender ON message_archive(sender_id,created_at);
            CREATE TABLE IF NOT EXISTS private_chats(chat_id INTEGER PRIMARY KEY,peer_id INTEGER,peer_name TEXT,peer_username TEXT,last_message_at INTEGER,imported_at INTEGER);
            ...
            CREATE INDEX IF NOT EXISTS idx_ops_user_time ON operation_log(user_id,created_at DESC);
            CREATE INDEX IF NOT EXISTS idx_ops_action_time ON operation_log(action,created_at DESC);
            CREATE TABLE IF NOT EXISTS cleanup_policy(category TEXT PRIMARY KEY,enabled INTEGER NOT NULL DEFAULT 1,retention_seconds INTEGER NOT NULL DEFAULT 2592000,custom_seconds INTEGER);
            CREATE TABLE IF NOT EXISTS broadcast_jobs(id INTEGER PRIMARY KEY AUTOINCREMENT,owner_id INTEGER,status TEXT,created_at INTEGER,started_at INTEGER,finished_at INTEGER,total INTEGER,sent INTEGER,failed INTEGER,text TEXT);
            CREATE TABLE IF NOT EXISTS storage_items(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,kind TEXT,path TEXT,caption TEXT,category TEXT,created_at INTEGER NOT NULL);
            CREATE INDEX IF NOT EXISTS idx_storage_kind_time ON storage_items(kind,created_at DESC);
            ''')
    ...
    def request_access(self,uid):
        r=self.one("SELECT id FROM access_requests WHERE user_id=? AND status='pending' ORDER BY id DESC LIMIT 1",(uid,))
        if r:return int(r['id']),False
        now=int(time.time())
        with self.conn() as c:
            cur=c.execute('INSERT INTO access_requests(user_id,created_at) VALUES(?,?)',(uid,now)); return int(cur.lastrowid),True
    def pending_requests(self,limit=25,offset=0): return self.all("SELECT * FROM access_requests WHERE status='pending' ORDER BY created_at LIMIT ? OFFSET ?",(limit,offset))
    def decide_request(self,rid,uid,accept,expires_at=None,reason=None):
        ...
        with self.conn() as c:
            c.execute('UPDATE access_requests SET status=?,reason=?,handled_at=? WHERE id=?',(status,reason,now,rid))
            c.execute("UPDATE users SET status=?,expires_at=?,updated_at=? WHERE id=?",('active' if accept else 'rejected',expires_at if accept else None,now,uid))
    def restrict(self,uid,seconds=None,permanent=False,reason=''):
        until=None if permanent else int(time.time())+int(seconds or 0)
        self.run('INSERT INTO restrictions(user_id,until_at,permanent,reason,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET until_at=excluded.until_at,permanent=excluded.permanent,reason=excluded.reason,updated_at=excluded.updated_at',(uid,until,int(permanent),reason,int(time.time())))
    ...
    def consume_link(self,token,requesting_user_id=None):
        ...
        if requesting_user_id is not None and int(r['user_id'])!=int(requesting_user_id): return None
        self.run('UPDATE temp_links SET uses=uses+1 WHERE token=?',(token,)); return int(r['user_id']),int(r['grant_seconds'])
    ...
    def keyboard_abuse(self,uid):
        r=self.one('SELECT attempts,updated_at FROM keyboard_abuse WHERE user_id=?',(uid,));
        if not r:return 0
        if int(time.time())-int(r['updated_at'])>3600:
            self.run('DELETE FROM keyboard_abuse WHERE user_id=?',(uid,)); return 0
    ...
    def stats(self):
        qs={'users':'SELECT COUNT(*) FROM users','active':'SELECT COUNT(*) FROM users WHERE status="active"','requests':'SELECT COUNT(*) FROM access_requests WHERE status="pending"','muted':'SELECT COUNT(*) FROM muted_users','logs':'SELECT COUNT(*) FROM operation_log','messages':'SELECT COUNT(*) FROM message_archive','storage':'SELECT COUNT(*) FROM storage_items','broadcasts':'SELECT COUNT(*) FROM broadcast_jobs'}
        return {k:self.one(q)[0] for k,q in qs.items()}
    def cleanup_expired(self):
        now=int(time.time()); self.run('DELETE FROM temp_links WHERE expires_at<=?',(now,)); self.run('DELETE FROM restrictions WHERE permanent=0 AND until_at<=?',(now,)); self.run("UPDATE users SET status='expired' WHERE status='active' AND expires_at IS NOT NULL AND expires_at<=?",(now,))
    def logs(self,limit=25,offset=0,user_id=None,action=None):
        q='SELECT * FROM operation_log WHERE 1=1'; p=[]
        if user_id is not None:q+=' AND user_id=?';p.append(user_id)
        if action:q+=' AND action=?';p.append(action)
        q+=' ORDER BY created_at DESC LIMIT ? OFFSET ?';p.extend([limit,offset]);return self.all(q,p)
    def cleanup_category(self,category,older_than):
        cutoff=int(time.time())-older_than
        mapping={'operations':'operation_log','private_log':'message_archive','access':'access_requests','broadcast':'broadcast_jobs','storage':'storage_items'}
        table=mapping.get(category)
        if not table:return 0
        col='created_at'
        return self.run(f'DELETE FROM {table} WHERE {col}<?',(cutoff,))
    def cleanup_counts(self):
        return {'operations':self.one('SELECT COUNT(*) FROM operation_log')[0],'private_log':self.one('SELECT COUNT(*) FROM message_archive')[0],'access':self.one('SELECT COUNT(*) FROM access_requests')[0],'broadcast':self.one('SELECT COUNT(*) FROM broadcast_jobs')[0],'storage':self.one('SELECT COUNT(*) FROM storage_items')[0]}
