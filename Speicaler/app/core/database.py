...
def _migrate_legacy_owner_data(owner_id):
    ...
    if owner_path.resolve() == GLOBAL_DB_PATH.resolve():
        return
    with get_global_connection() as src, sqlite3.connect(owner_path, timeout=30) as dst:
        dst.row_factory = sqlite3.Row
        tables = [
            'muted_users','chat_muted_users','private_chats','messages','message_edit_history',
            'important_messages','broadcast_exclusions','broadcast_logs','scheduled_broadcasts',
            'ai_logs','security_log','operation_log','settings'
        ]
        for table in tables:
            try:
            ...
            for row in rows:
                ...
                try:
                    verb = 'INSERT OR REPLACE' if table == 'settings' else 'INSERT OR IGNORE'
                    dst.execute(f"{verb} INTO {table}({','.join(cols)}) VALUES({placeholders})", values)
...
def get_setting(key, default=None):
    conn = get_global_connection if key in GLOBAL_SETTING_KEYS else get_connection
    with conn() as c:
...
def mark_message_edited(telegram_message_id, chat_id, direction, new_text):
    ...
    return record_edit(telegram_message_id, chat_id, row["user_id"] if row else 0, direction, old_text, new_text)
...
def list_private_chats(include_bots=False):
    ...
    sql += " ORDER BY archived ASC,last_seen DESC"
    with get_connection() as c:
...
def private_chats_page(page=0, per_page=10):
    ...
    with get_connection() as c:
        ...
        rows = c.execute(
            "SELECT * FROM private_chats WHERE is_bot=0 AND is_deleted_user=0 ORDER BY archived ASC, last_seen DESC, display_name COLLATE NOCASE ASC LIMIT ? OFFSET ?",
            (per_page, offset),
        ).fetchall()
