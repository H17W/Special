...
def init_database():
    ...
    with get_connection() as c:
        ...
        CREATE TABLE IF NOT EXISTS chat_muted_users(chat_id INTEGER NOT NULL,user_id INTEGER NOT NULL,username TEXT,display_name TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(chat_id,user_id));
...
def mute_user_in_chat(chat_id,user_id,username=None,display_name=None):
    with get_connection() as c:
        c.execute("INSERT OR REPLACE INTO chat_muted_users(chat_id,user_id,username,display_name) VALUES(?,?,?,?)",(chat_id,user_id,username,display_name)); c.commit()

def unmute_user_in_chat(chat_id,user_id):
    with get_connection() as c:
        cur=c.execute("DELETE FROM chat_muted_users WHERE chat_id=? AND user_id=?",(chat_id,user_id)); c.commit(); return cur.rowcount>0

def is_user_muted_in_chat(chat_id,user_id):
    with get_connection() as c:
        return c.execute("SELECT 1 FROM chat_muted_users WHERE chat_id=? AND user_id=?",(chat_id,user_id)).fetchone() is not None
...
def get_messages_by_telegram_ids(telegram_ids, chat_id=None):
    ...
    if chat_id is not None:
        sql += " AND m.chat_id=?"
        params=ids+[chat_id]
    with get_connection() as c:return c.execute(sql,params).fetchall()
