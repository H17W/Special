...
from app.user.moderation import check_muted, mute, unmute, is_muted_in_chat
...
async def archive_media(event, user_id, direction):
    ...
    try:
        return await event.download_media(
            file=str(folder / f"{direction}_{event.id}")
        )
    except Exception:
        return None
...
async def save_replied_media(event, replied, sender):
    ...
    try:
        path = await replied.download_media(
            file=str(temp_dir / f"media_{replied.id}")
        )
...
def _load_profile_backup():
    ...
    try:
        return json.loads(
            PROFILE_BACKUP.read_text(encoding="utf-8")
        )
    except Exception:
        return None
...
async def _delete_current_profile_photos():
    try:
        ...
        add_log(
            ...
            f"فشل حذف صور الملف الشخصي: {type(exc).__name__}",
        )
...
async def mimic_user(event, target):
    ...
    if photos:
        temp_dir = Path(
            tempfile.mkdtemp(prefix="special_mimic_")
        )

        try:
            path = await client.download_media(
                photos[0],
                file=str(temp_dir / "target_profile"),
            )
    ...
    add_log(
        "operation_log",
        "profile_mimic_started",
        f"تم تقليد المستخدم {target.id}",
    )
...
async def restore_profile():
    ...
    add_log(
        "operation_log",
        "profile_mimic_stopped",
        "تم استعادة بيانات الملف الشخصي الأصلية",
    )
...
async def has_delete_permission(chat_id):
    try:
        ...
        if getattr(entity, "class_name", "") == "User":
            return True
...
async def send_deleted_record_to_bot(row):
    ...
    info = (
        ...
        f"🆔 ID: {user_id}\n"
        f"🔗 Username: "
        f"{('@' + username) if username else 'لا يوجد يوزر'}"
    )
...
async def incoming(event):
    ...
    if event.is_private:
        ...
        increment_stats(
            sender.id,
            mt,
            has_link(event.raw_text),
        )

    if check_muted(sender.id) or is_muted_in_chat(
        event.chat_id,
        sender.id,
    ):
        try:
...
async def incoming_edit(event):
    ...
    add_log(
        "operation_log",
        "message_edited",
        f"تم تعديل رسالة واردة ID={event.id} في chat={event.chat_id}",
    )
...
async def outgoing_edit(event):
    ...
    add_log(
        "operation_log",
        "message_edited",
        f"تم تعديل رسالة صادرة ID={event.id} في chat={event.chat_id}",
    )
...
async def outgoing(event):
    ...
    if event.is_private:
        ...
        media_path = await archive_media(
            event,
            target.id,
            "out",
        )
    ...
    if text in {"/بداية", "بداية"}:
        if not event.is_reply:
            await event.edit(
                "❌ فشلت العملية يجب الرد على الرسالة للتحديد"
            )
            return
    ...
    if text in {"/نهاية", "نهاية"}:
        ...
        if start_id is None:
            await event.edit(
                "❌ فشلت العملية يجب تحديد رسالة البداية أولًا"
            )
            return
        ...
        if end_id < start_id:
            await event.edit(
                "❌ فشلت العملية يجب أن تكون النهاية بعد البداية"
            )
            return

        try:
            count = await delete_message_range(
                event.chat_id,
                start_id,
                end_id,
            )
            ...
            await event.edit(
                "❌ تعذر المسح ليس لديك صلاحية حذف رسائل الاخرين"
            )
            ...
            await event.edit(
                "❌ تعذر المسح حدث خطأ أثناء حذف الرسائل"
            )
    ...
    if text in {
        ...
        if not event.is_private:
            await event.edit(
                "❌ حفظ المهم متاح حاليًا في الخاص فقط"
            )
            return

        if not event.is_reply:
            await event.edit(
                "❌ فشلت العملية يجب الرد على الرسالة لحفظها كمهمة"
            )
            return
        ...
        if mt3:
            ...
            try:
                media3 = await replied.download_media(
                    file=str(
                        folder /
                        f"important_{replied.id}"
                    )
                )
        ...
        local_id3 = add_message(
            replied.id,
            uid3,
            event.chat_id,
            "incoming" if not replied.out else "outgoing",
            replied.raw_text,
            mt3,
            media3,
            str(replied.date),
        ...
        if local_id3:
            ...
            await event.edit(
                "⭐ تم حفظ الرسالة كمهمة بنجاح"
            )
        else:
            await event.edit(
                "❌ تعذر حفظ الرسالة المهمة"
            )
    ...
    if text in {
        ...
        if not target:
            await event.edit(
                "❌ فشلت العملية يجب الرد على المستخدم أو كتابة منشنه"
            )
            return
        ...
        if existing and existing.get("active"):
            await event.edit(
                "ℹ️ التقليد مفعّل بالفعل ألغِ التقليد أولًا"
            )
            return
        ...
        try:
            ...
            await event.edit(
                f"✅ تم تقليد {target_name} بنجاح"
            )

        except Exception as exc:
            await event.edit(
                f"❌ تعذر التقليد: {type(exc).__name__}"
            )
    ...
    if text in {
        ...
        try:
            if await restore_profile():
                await event.edit(
                    "✅ تم إلغاء التقليد واستعادة حسابك مثل ما كان"
                )
            else:
                await event.edit(
                    "ℹ️ ما فيه تقليد مفعّل حاليًا"
                )

        except Exception as exc:
            await event.edit(
                f"❌ تعذر إلغاء التقليد: {type(exc).__name__}"
            )
    ...
    if text in {
        ...
        name2 = (
            getattr(target2, "first_name", None)
            or getattr(target2, "last_name", None)
            or getattr(target2, "title", None)
            or "مستخدم"
        )
        ...
        if (
            ...
            and not await has_delete_permission(
                event.chat_id
            )
        ):
            await event.edit(
        ...
        await event.edit(
            ...
            f"ID: {target2.id}\n"
            "Username: "
            f"{('@' + user2) if user2 else 'لا يوجد يوزر'}"
        )
...
async def start_user_client():
    ...
    for attempt in range(1, 6):
        try:
            ...
            print(
                f"⚠️ جلسة Telethon مقفولة — "
                f"المحاولة {attempt}/5"
            )
